...
impl CryptoBackend for RingBackend {
    fn verify_signature(
        ...
        log::trace!("TBS data length: {}", tbs_data.len());
        log::trace!("Signature length: {}", signature.len());
        log::trace!("Public key length: {}", public_key.len());

        // Map our algorithm enum to ring's signature verification algorithm
        ...
        pk.verify(tbs_data, signature).map_err(|e| {
