...
impl Time {
    ...
    ///
    /// This is used for time comparisons.
...
impl der::FixedTag for Time {
    // Time uses CHOICE, so it doesn't have a fixed tag
    // We use UtcTime as the default tag for encoding purposes
...
impl Validity {
    ...
    ///
    /// Returns `true` if `check_time` is within the validity period,
    /// i.e., not_before <= check_time <= not_after.
    ...
    ///
    /// Returns `true` if not_before <= not_after.
...
/// # Arguments
///
/// * `validity` - The certificate validity period to check
/// * `check_time` - The time to check against
///
/// # Returns
///
/// `true` if the certificate is valid at `check_time`, `false` otherwise.
///
/// # Example
///
/// ```ignore
/// use der::asn1::UtcTime;
/// use spdm_x509::time_utils::{Time, Validity, is_valid_at};
///
...
///
/// let check_time = Time::UtcTime(UtcTime::from_unix_duration(...)?);
/// if is_valid_at(&validity, &check_time) {
...
///
/// This function checks the validity against the current system time.
...
///
/// ```ignore
/// use spdm_x509::time_utils::{Validity, is_currently_valid};
///
/// if is_currently_valid(&validity) {
///     println!("Certificate is currently valid!");
/// }
/// ```
#[cfg(feature = "std")]
pub fn is_currently_valid(validity: &Validity) -> bool {
    use der::DateTime;
    ...
    // Convert to DateTime
    if let Ok(now_dt) = DateTime::from_unix_duration(now) {
        // Create a Time from the current DateTime
        // Use GeneralizedTime for current time as it can represent any date
        let gen_time = GeneralizedTime::from_date_time(now_dt);
        let current_time = Time::GeneralizedTime(gen_time);
        return validity.is_valid_at(&current_time);
    }
...
mod tests {
    ...
    fn test_time_comparison() {
        // Create two UTC times for testing
        // Using from_unix_duration to create specific times
        use der::asn1::UtcTime;
