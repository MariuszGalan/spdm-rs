    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            ParseError::InvalidTag { expected, found } => {
                write!(f, "Invalid tag: expected 0x{:02x}, found 0x{:02x}", expected, found)
            }
...
impl fmt::Display for SignatureError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            SignatureError::AlgorithmMismatch { cert_algo, tbs_algo } => {
                write!(f, "Signature algorithm mismatch: cert={}, tbs={}", cert_algo, tbs_algo)
            }
            ...
            SignatureError::InvalidSignatureLength { expected, found } => {
                write!(f, "Invalid signature length: expected {}, found {}", expected, found)
            }
...
impl fmt::Display for TimeError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            TimeError::InvalidValidityPeriod { not_before, not_after } => {
                write!(f, "Invalid validity period: notBefore={}, notAfter={}", not_before, not_after)
            }
...
impl fmt::Display for ExtensionError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            ExtensionError::InvalidEncoding(msg) => write!(f, "Invalid extension encoding: {}", msg),
            ...
            ExtensionError::SubjectAltName(msg) => write!(f, "Subject Alternative Name error: {}", msg),
    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            EncodingError::InvalidPemLabel { expected, found } => {
                write!(f, "Invalid PEM label: expected '{}', found '{}'", expected, found)
            }
    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ConstraintError::PathLength { max, actual } => {
                write!(f, "Path length constraint violated: max={}, actual={}", max, actual)
            }
            ConstraintError::NotCa => write!(f, "Certificate is not a CA but used as one"),
            ConstraintError::UnexpectedCa => write!(f, "Certificate is a CA but used as end-entity"),
...
// ============================================================================
// std::error::Error Implementation (only when std feature is enabled)
// ============================================================================
...
/// Convert from algorithms module errors
impl From<crate::algorithms::AlgorithmError> for Error {
    fn from(err: crate::algorithms::AlgorithmError) -> Self {
        Error::AlgorithmError(AlgorithmError::Unsupported(err.to_string()))
    }
}
...
mod tests {
    ...
    fn test_error_display() {
        ...
        assert_eq!(err.to_string(), "Signature error: Signature verification failed");
    ...
    fn test_helper_constructors() {
        ...
        assert!(matches!(err, Error::AlgorithmError(AlgorithmError::Unsupported(_))));

        let err = Error::weak_key("RSA", 512);
        assert!(matches!(err, Error::KeyError(KeyError::WeakKey { bits: 512, .. })));
    }
