...
pub fn verify_cert_chain(cert_chain: &[u8]) -> Result<()> {
    log::trace!(
        "[spdm-x509-rs] verify_cert_chain: chain_len={}",
        cert_chain.len()
    );
    ...
    loop {
        match get_cert_from_cert_chain(cert_chain, cert_index) {
            Ok((start, end)) => {
                log::trace!(
                    "[spdm-x509-rs] verify_cert_chain: parsed cert {} at [{}, {})",
                    cert_index,
                    start,
                    end
                );
                ...
                log::trace!(
                    "[spdm-x509-rs] verify_cert_chain: cert {} subject={:?}",
                    cert_index,
                    cert.tbs_certificate.subject
                );
                log::trace!(
                    "[spdm-x509-rs] verify_cert_chain: cert {} issuer={:?}",
                    cert_index,
            ...
            Err(e) => {
                log::trace!(
                    "[spdm-x509-rs] verify_cert_chain: no more certs at index {} (error: {:?})",
                    cert_index,
                    e
                );
                break;
        ...
        if cert_index > 100 {
            log::error!("[spdm-x509-rs] verify_cert_chain: chain too long (>100)");
            return Err(Error::ValidationError(
                "Certificate chain too long (>100 certificates)".into(),
            ));
        }
    ...
    if certificates.is_empty() {
        log::error!("[spdm-x509-rs] verify_cert_chain: empty chain");
        return Err(Error::ChainError(ChainError::EmptyChain));
    }

    log::trace!(
        "[spdm-x509-rs] verify_cert_chain: parsed {} certificates, building chain",
        certificates.len()
    );
    ...
    log::trace!("[spdm-x509-rs] verify_cert_chain: reversed chain for validation (leaf → root)");
    ...
    log::trace!("[spdm-x509-rs] verify_cert_chain: validating chain with standard X.509 validator");
    ...
    log::trace!(
        "[spdm-x509-rs] verify_cert_chain: validating leaf certificate EKU (Responder role)"
    );
    ...
    if !chain.is_empty() {
        ...
        {
            Ok(_) => {
                log::trace!("[spdm-x509-rs] verify_cert_chain: ✓ SPDM EKU validation successful");
            }
            Err(e) => {
                log::error!(
                    "[spdm-x509-rs] verify_cert_chain: ✗ SPDM EKU validation failed: {:?}",
                    e
                );
                return Err(e);
            }
    ...
    log::trace!("[spdm-x509-rs] verify_cert_chain: ✓ validation successful");
    Ok(())
}
